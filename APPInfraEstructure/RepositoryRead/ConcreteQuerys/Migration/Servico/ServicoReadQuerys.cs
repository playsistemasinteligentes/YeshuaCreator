...
namespace Query.Read 
{
    public class ServicoQueryRead : QueryBase, IServicoQueryRead
    {
        ...
        public QueryModel ServicoQuery(Command.Read.ServicoReadCommand Command)
        {
            ...
            this.Query = $@" select Id, GrupoServicoId, Nome, Valor, TenantID, Deleted, UserId from Servico ";
...
if (!string.IsNullOrEmpty(Command.Nome)) whereClauses.Add($"Nome like @Nome");
        ...
        {
            var sql = $"SELECT 1 FROM Servico WHERE {getBackEndFieldWitchWhere()} Id = @Id";
            var parameters = new { Id = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel ExistsByGrupoServicoIdQuery(int value)
        {
            var sql = $"SELECT 1 FROM Servico WHERE {getBackEndFieldWitchWhere()} GrupoServicoId = @GrupoServicoId";
            var parameters = new { GrupoServicoId = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel ExistsByNomeQuery(string value)
        {
            var sql = $"SELECT 1 FROM Servico WHERE {getBackEndFieldWitchWhere()} Nome = @Nome";
            var parameters = new { Nome = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel ExistsByValorQuery(Decimal value)
        {
            var sql = $"SELECT 1 FROM Servico WHERE {getBackEndFieldWitchWhere()} Valor = @Valor";
            var parameters = new { Valor = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel ExistsByUserIdQuery(int value)
        {
            var sql = $"SELECT 1 FROM Servico WHERE {getBackEndFieldWitchWhere()} UserId = @UserId";
            var parameters = new { UserId = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel FirstByIdQuery(int value)
        {
            var sql = $"SELECT * FROM Servico WHERE {getBackEndFieldWitchWhere()} Id = @Id";
            var parameters = new { Id = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel FirstByGrupoServicoIdQuery(int value)
        {
            var sql = $"SELECT * FROM Servico WHERE {getBackEndFieldWitchWhere()} GrupoServicoId = @GrupoServicoId";
            var parameters = new { GrupoServicoId = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel FirstByNomeQuery(string value)
        {
            var sql = $"SELECT * FROM Servico WHERE {getBackEndFieldWitchWhere()} Nome = @Nome";
            var parameters = new { Nome = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel FirstByValorQuery(Decimal value)
        {
            var sql = $"SELECT * FROM Servico WHERE {getBackEndFieldWitchWhere()} Valor = @Valor";
            var parameters = new { Valor = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel FirstByUserIdQuery(int value)
        {
            var sql = $"SELECT * FROM Servico WHERE {getBackEndFieldWitchWhere()} UserId = @UserId";
            var parameters = new { UserId = value };
            return new QueryModel(sql, parameters);
        }
