...
namespace Query.Read 
{
    public class GrupoServicoQueryRead : QueryBase, IGrupoServicoQueryRead
    {
        ...
        public QueryModel GrupoServicoQuery(Command.Read.GrupoServicoReadCommand Command)
        {
            ...
            this.Query = $@" select Id, Descricao, TenantID, Deleted, UserId from GrupoServico ";
...
if (!string.IsNullOrEmpty(Command.Descricao)) whereClauses.Add($"Descricao like @Descricao");
        ...
        {
            var sql = $"SELECT 1 FROM GrupoServico WHERE {getBackEndFieldWitchWhere()} Id = @Id";
            var parameters = new { Id = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel ExistsByDescricaoQuery(string value)
        {
            var sql = $"SELECT 1 FROM GrupoServico WHERE {getBackEndFieldWitchWhere()} Descricao = @Descricao";
            var parameters = new { Descricao = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel ExistsByUserIdQuery(int value)
        {
            var sql = $"SELECT 1 FROM GrupoServico WHERE {getBackEndFieldWitchWhere()} UserId = @UserId";
            var parameters = new { UserId = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel FirstByIdQuery(int value)
        {
            var sql = $"SELECT * FROM GrupoServico WHERE {getBackEndFieldWitchWhere()} Id = @Id";
            var parameters = new { Id = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel FirstByDescricaoQuery(string value)
        {
            var sql = $"SELECT * FROM GrupoServico WHERE {getBackEndFieldWitchWhere()} Descricao = @Descricao";
            var parameters = new { Descricao = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel FirstByUserIdQuery(int value)
        {
            var sql = $"SELECT * FROM GrupoServico WHERE {getBackEndFieldWitchWhere()} UserId = @UserId";
            var parameters = new { UserId = value };
            return new QueryModel(sql, parameters);
        }
