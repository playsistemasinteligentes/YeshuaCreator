...
namespace Query.Read 
{
    public class yUserModuleQueryRead : QueryBase, IyUserModuleQueryRead
    {
        ...
        public yUserModuleQueryRead(ICurrentUser correntUser)
        {
        ...
        public QueryModel yUserModuleQuery(Command.Read.yUserModuleReadCommand Command )
        {
            ...
            this.Query = $@" select Id, ModuleId, UserId, ValidUntil, TenantID, Deleted, Changed from yUserModule ";
if (Command.Id.HasValue) parametersDict["Id"] = Command.Id.Value;
if (Command.Id.HasValue) whereClauses.Add($"Id = @Id");
if (!string.IsNullOrEmpty(Command.ModuleId)) parametersDict["ModuleId"] = $"%{Command.ModuleId}%";
...
if (Command.UserId.HasValue) whereClauses.Add($"UserId = @UserId");
        ...
        public QueryModel yUserModuleModuleIdQuery(Command.Patterns.Command.SearchFKCommand Command )
        {
            ...
            if (!string.IsNullOrEmpty(Command.searchFK)) 
            {
                 ...
                 else 
                 {
                      ...
                      whereClauses.Add(" id in (select ModuleId from yTenantModule where TenantID = _correntUser.TenantID) "); 
            if (whereClauses.Any()) 
            this.Query += $" WHERE ({string.Join(" AND ", whereClauses)})"; 
            this.Parameters = parameters;
            return new QueryModel(this.Query, this.Parameters); 
 ...
 parametersDict["Deleted"] = 0;
 whereClauses.Add($"Deleted = @Deleted");
                      parametersDict["ModuleId"] = value; 
                      whereClauses.Add($" ModuleId = @ModuleId ");
            if (whereClauses.Any()) 
            this.Query += $" WHERE ({string.Join(" AND ", whereClauses)})"; 
            this.Parameters = parameters;
            return new QueryModel(this.Query, parameters);
