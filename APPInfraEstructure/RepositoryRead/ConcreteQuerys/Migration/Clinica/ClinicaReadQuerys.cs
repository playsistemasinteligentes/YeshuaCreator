...
namespace Query.Read 
{
    public class ClinicaQueryRead : QueryBase, IClinicaQueryRead
    {
        ...
        public QueryModel ClinicaQuery(Command.Read.ClinicaReadCommand Command)
        {
            ...
            this.Query = $@" select Id, Nome, Endereco, Telefone, TenantID, Deleted, UserId from Clinica ";
...
if (!string.IsNullOrEmpty(Command.Telefone)) whereClauses.Add($"Telefone like @Telefone");
        ...
        {
            var sql = $"SELECT 1 FROM Clinica WHERE {getBackEndFieldWitchWhere()} Id = @Id";
            var parameters = new { Id = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel ExistsByNomeQuery(string value)
        {
            var sql = $"SELECT 1 FROM Clinica WHERE {getBackEndFieldWitchWhere()} Nome = @Nome";
            var parameters = new { Nome = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel ExistsByEnderecoQuery(string value)
        {
            var sql = $"SELECT 1 FROM Clinica WHERE {getBackEndFieldWitchWhere()} Endereco = @Endereco";
            var parameters = new { Endereco = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel ExistsByTelefoneQuery(string value)
        {
            var sql = $"SELECT 1 FROM Clinica WHERE {getBackEndFieldWitchWhere()} Telefone = @Telefone";
            var parameters = new { Telefone = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel ExistsByUserIdQuery(int value)
        {
            var sql = $"SELECT 1 FROM Clinica WHERE {getBackEndFieldWitchWhere()} UserId = @UserId";
            var parameters = new { UserId = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel FirstByIdQuery(int value)
        {
            var sql = $"SELECT * FROM Clinica WHERE {getBackEndFieldWitchWhere()} Id = @Id";
            var parameters = new { Id = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel FirstByNomeQuery(string value)
        {
            var sql = $"SELECT * FROM Clinica WHERE {getBackEndFieldWitchWhere()} Nome = @Nome";
            var parameters = new { Nome = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel FirstByEnderecoQuery(string value)
        {
            var sql = $"SELECT * FROM Clinica WHERE {getBackEndFieldWitchWhere()} Endereco = @Endereco";
            var parameters = new { Endereco = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel FirstByTelefoneQuery(string value)
        {
            var sql = $"SELECT * FROM Clinica WHERE {getBackEndFieldWitchWhere()} Telefone = @Telefone";
            var parameters = new { Telefone = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel FirstByUserIdQuery(int value)
        {
            var sql = $"SELECT * FROM Clinica WHERE {getBackEndFieldWitchWhere()} UserId = @UserId";
            var parameters = new { UserId = value };
            return new QueryModel(sql, parameters);
        }
