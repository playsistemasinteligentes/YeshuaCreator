...
{
    public class YpermissionModulesQueryRead : QueryBase, IYpermissionModulesQueryRead
    {
        protected readonly ICurrentUser _correntUser;
        public YpermissionModulesQueryRead(ICurrentUser correntUser)
        {
        ...
        public QueryModel YpermissionModulesQuery(Command.Read.YpermissionModulesReadCommand Command)
        {
            ...
            this.Query = $@" select Id, Description, TenantID, Deleted, Changed, UserId from YpermissionModules ";
...
if (!string.IsNullOrEmpty(Command.Description)) whereClauses.Add($"Description like @Description");
        ...
        {
            var sql = $"SELECT 1 FROM YpermissionModules WHERE {getBackEndFieldWitchWhere(" AND ")} Id = @Id";
            var parameters = new { Id = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel ExistsByDescriptionQuery(string value)
        {
            var sql = $"SELECT 1 FROM YpermissionModules WHERE {getBackEndFieldWitchWhere(" AND ")} Description = @Description";
            var parameters = new { Description = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel ExistsByChangedQuery(DateTime value)
        {
            var sql = $"SELECT 1 FROM YpermissionModules WHERE {getBackEndFieldWitchWhere(" AND ")} Changed = @Changed";
            var parameters = new { Changed = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel ExistsByUserIdQuery(int value)
        {
            var sql = $"SELECT 1 FROM YpermissionModules WHERE {getBackEndFieldWitchWhere(" AND ")} UserId = @UserId";
            var parameters = new { UserId = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel FirstByIdQuery(string value)
        {
            var sql = $"SELECT * FROM YpermissionModules WHERE {getBackEndFieldWitchWhere(" AND ")}  Id = @Id";
            var parameters = new { Id = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel FirstByDescriptionQuery(string value)
        {
            var sql = $"SELECT * FROM YpermissionModules WHERE {getBackEndFieldWitchWhere(" AND ")}  Description = @Description";
            var parameters = new { Description = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel FirstByChangedQuery(DateTime value)
        {
            var sql = $"SELECT * FROM YpermissionModules WHERE {getBackEndFieldWitchWhere(" AND ")}  Changed = @Changed";
            var parameters = new { Changed = value };
            return new QueryModel(sql, parameters);
        }
        public QueryModel FirstByUserIdQuery(int value)
        {
            var sql = $"SELECT * FROM YpermissionModules WHERE {getBackEndFieldWitchWhere(" AND ")}  UserId = @UserId";
            var parameters = new { UserId = value };
            return new QueryModel(sql, parameters);
        }
