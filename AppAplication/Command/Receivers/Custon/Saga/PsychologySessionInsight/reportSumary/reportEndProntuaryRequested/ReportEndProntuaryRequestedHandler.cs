...
namespace Command.Receivers
{
    public partial class ReportEndProntuaryRequestedHandler
    {
        ...
        partial void CustomExecute(SagaBase saga, SagaStepBase step)
        {
            ...
            _logger.Info($"[STEP_2] Payload enviado para sumarização — sessão {sessaoId}.");
        }
        ...
        private static object BuildPayload(int sessaoId, string stepPayload)
        {
            ...
            new
            {
                key         = "Prontuario",
                group       = "Atendimento",
                instruction = "Com base na transcrição, elabore um prontuário clínico estruturado da sessão",
                maxLength   = 8000,
                type        = "text"
            },
            new
            {
                key         = "QueixaPrincipal",
                group       = "Atendimento",
                instruction = "Identifique e resuma a queixa principal relatada pelo paciente nesta sessão",
                maxLength   = 1000,
                type        = "text"
            },
            new
            {
                ...
                group       = "Atendimento",
                instruction = "Elabore o registro documental formal da sessão conforme padrão CRP",
                maxLength   = 8000,
                type        = "text"
            }
        }
        ...
        partial void CustomApplyResponse(SagaBase saga, SagaStepBase step, string payload)
        {
            ...
            if (response?.Fields == null)
                throw new Exception($"[STEP_2] Payload inválido ou nulo ao aplicar resposta na sessão {sessaoId}.");
            ...
            _logger.Info($"[STEP_2] Campos aplicados com sucesso — sessão {sessaoId}.");
        }
    }
