...
namespace Yeshua.APS.ADM.CQRS.Tests.Integration.Api.Smoke.Migration.Cabvisao;

[SmokeTestOrder(113)]
public partial class CabvisaoCrudApiSmokeTests : ApiIntegrationTestBase
{
    ...
    private const string DeleteEndpoint = "yapi/Cabvisao/DeleteCabvisao";
    ...
    public async Task ExecuteAsync()
    {
        ...
        ApiSmokeTestContext.RegisterCreatedId("Cabvisao", createdId);

        var initialDeletePayload = BuildDeletePayload(createPayload, createdId);
        CustomizeDeletePayload(initialDeletePayload);
        ApiSmokeTestContext.RegisterDeletePayload("Cabvisao", initialDeletePayload);
        ...
        ApiSmokeTestContext.RegisterDeletePayload("Cabvisao", deletePayload);
    }

    public async Task DeleteAsync()
    {
        if (!ApiSmokeTestContext.TryGetDeletePayload("Cabvisao", out var deletePayload))
            return;

        using var client = await CreateAuthenticatedClientAsync();
        using var deleteResponse = await client.SendJsonAsync(HttpMethod.Delete, DeleteEndpoint, deletePayload, JsonOptions);
        var deleteState = await ApiResponseAssertions.ReadSuccessStateAsync(deleteResponse);
        var deletedId = ApiJson.GetRequiredProperty(deleteState, "data", "cab_id");
        var expectedId = ApiJson.GetRequiredProperty(deletePayload, "CAB_ID");
