...
namespace Yeshua.APS.ADM.CQRS.Tests.Integration.Api.Smoke.Migration.T_Metas;

[SmokeTestOrder(152)]
public partial class T_MetasCrudApiSmokeTests : ApiIntegrationTestBase
{
    ...
    private const string DeleteEndpoint = "yapi/T_Metas/DeleteT_Metas";
    ...
    public async Task ExecuteAsync()
    {
        ...
        ApiSmokeTestContext.RegisterCreatedId("T_Metas", createdId);

        var initialDeletePayload = BuildDeletePayload(createPayload, createdId);
        CustomizeDeletePayload(initialDeletePayload);
        ApiSmokeTestContext.RegisterDeletePayload("T_Metas", initialDeletePayload);
        ...
        if (!readAssertionHandled)
            ApiResponseAssertions.AssertReadContainsId(readState, createdId, "met_id");
        ...
        ApiSmokeTestContext.RegisterDeletePayload("T_Metas", deletePayload);
    }

    public async Task DeleteAsync()
    {
        if (!ApiSmokeTestContext.TryGetDeletePayload("T_Metas", out var deletePayload))
            return;

        using var client = await CreateAuthenticatedClientAsync();
        using var deleteResponse = await client.SendJsonAsync(HttpMethod.Delete, DeleteEndpoint, deletePayload, JsonOptions);
        var deleteState = await ApiResponseAssertions.ReadSuccessStateAsync(deleteResponse);
        var deletedId = ApiJson.GetRequiredProperty(deleteState, "data", "met_id");
        var expectedId = ApiJson.GetRequiredProperty(deletePayload, "MET_ID");
