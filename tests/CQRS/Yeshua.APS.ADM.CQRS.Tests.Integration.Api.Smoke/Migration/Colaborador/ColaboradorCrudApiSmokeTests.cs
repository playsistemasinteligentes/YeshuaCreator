...
public partial class ColaboradorCrudApiSmokeTests : ApiIntegrationTestBase
{
    ...
    private const string DeleteEndpoint = "yapi/Colaborador/DeleteColaborador";
    ...
    public async Task ExecuteAsync()
    {
        ...
        ApiSmokeTestContext.RegisterCreatedId("Colaborador", createdId);

        var initialDeletePayload = BuildDeletePayload(createPayload, createdId);
        CustomizeDeletePayload(initialDeletePayload);
        ApiSmokeTestContext.RegisterDeletePayload("Colaborador", initialDeletePayload);
        ...
        ApiSmokeTestContext.RegisterDeletePayload("Colaborador", deletePayload);
    }

    public async Task DeleteAsync()
    {
        if (!ApiSmokeTestContext.TryGetDeletePayload("Colaborador", out var deletePayload))
            return;

        using var client = await CreateAuthenticatedClientAsync();
        using var deleteResponse = await client.SendJsonAsync(HttpMethod.Delete, DeleteEndpoint, deletePayload, JsonOptions);
        var deleteState = await ApiResponseAssertions.ReadSuccessStateAsync(deleteResponse);
        var deletedId = ApiJson.GetRequiredProperty(deleteState, "data", "col_cpf");
        var expectedId = ApiJson.GetRequiredProperty(deletePayload, "COL_CPF");
