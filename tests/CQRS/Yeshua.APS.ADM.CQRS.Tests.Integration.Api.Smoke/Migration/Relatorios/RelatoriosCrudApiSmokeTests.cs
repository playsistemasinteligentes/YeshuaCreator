...
namespace Yeshua.APS.ADM.CQRS.Tests.Integration.Api.Smoke.Migration.Relatorios;

[SmokeTestOrder(117)]
public partial class RelatoriosCrudApiSmokeTests : ApiIntegrationTestBase
{
    ...
    private const string DeleteEndpoint = "yapi/Relatorios/DeleteRelatorios";
    ...
    public async Task ExecuteAsync()
    {
        ...
        ApiSmokeTestContext.RegisterCreatedId("Relatorios", createdId);

        var initialDeletePayload = BuildDeletePayload(createPayload, createdId);
        CustomizeDeletePayload(initialDeletePayload);
        ApiSmokeTestContext.RegisterDeletePayload("Relatorios", initialDeletePayload);
        ...
        ApiSmokeTestContext.RegisterDeletePayload("Relatorios", deletePayload);
    }

    public async Task DeleteAsync()
    {
        if (!ApiSmokeTestContext.TryGetDeletePayload("Relatorios", out var deletePayload))
            return;

        using var client = await CreateAuthenticatedClientAsync();
        using var deleteResponse = await client.SendJsonAsync(HttpMethod.Delete, DeleteEndpoint, deletePayload, JsonOptions);
        var deleteState = await ApiResponseAssertions.ReadSuccessStateAsync(deleteResponse);
        var deletedId = ApiJson.GetRequiredProperty(deleteState, "data", "rel_id");
        var expectedId = ApiJson.GetRequiredProperty(deletePayload, "REL_ID");
