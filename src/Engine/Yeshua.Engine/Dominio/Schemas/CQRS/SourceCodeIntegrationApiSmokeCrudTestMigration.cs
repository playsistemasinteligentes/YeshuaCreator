...
{
    public class SourceCodeIntegrationApiSmokeCrudTestMigration : SourceCodeBase
    {
        ...
        protected override StringBuilder GenerateCode()
        {
            ...
            sb.AppendLine($"        ApiSmokeTestContext.RegisterCreatedId(\"{Escape(_entity.EntityName)}\", createdId);");
            sb.AppendLine();
            sb.AppendLine("        var initialDeletePayload = BuildDeletePayload(createPayload, createdId);");
            sb.AppendLine("        CustomizeDeletePayload(initialDeletePayload);");
            sb.AppendLine($"        ApiSmokeTestContext.RegisterDeletePayload(\"{Escape(_entity.EntityName)}\", initialDeletePayload);");
            sb.AppendLine();
            ...
            sb.AppendLine();
            sb.AppendLine("        var deletePayload = BuildDeletePayload(updatePayload, createdId);");
            sb.AppendLine("        CustomizeDeletePayload(deletePayload);");
            sb.AppendLine($"        ApiSmokeTestContext.RegisterDeletePayload(\"{Escape(_entity.EntityName)}\", deletePayload);");
            sb.AppendLine("    }");
            sb.AppendLine();
            sb.AppendLine("    public async Task DeleteAsync()");
            sb.AppendLine("    {");
            sb.AppendLine($"        if (!ApiSmokeTestContext.TryGetDeletePayload(\"{Escape(_entity.EntityName)}\", out var deletePayload))");
            sb.AppendLine("            return;");
            sb.AppendLine();
            sb.AppendLine("        using var client = await CreateAuthenticatedClientAsync();");
            sb.AppendLine("        using var deleteResponse = await client.SendJsonAsync(HttpMethod.Delete, DeleteEndpoint, deletePayload, JsonOptions);");
            sb.AppendLine("        var deleteState = await ApiResponseAssertions.ReadSuccessStateAsync(deleteResponse);");
            sb.AppendLine($"        var deletedId = ApiJson.GetRequiredProperty(deleteState, \"data\", \"{keyName.ToLowerInvariant()}\");");
            sb.AppendLine($"        var expectedId = ApiJson.GetRequiredProperty(deletePayload, \"{keyName}\");");
            sb.AppendLine("        ApiResponseAssertions.AssertSameJsonValue(expectedId, deletedId, \"deleted id\");");
            sb.AppendLine("    }");
            sb.AppendLine();
            ...
            sb.AppendLine("    }");
            sb.AppendLine();
            sb.AppendLine("    private static JsonObject BuildDeletePayload(JsonObject updatePayload, JsonNode id)");
            sb.AppendLine("    {");
            sb.AppendLine("        var payload = (JsonObject)updatePayload.DeepClone();");
            sb.AppendLine($"        payload[\"{keyName}\"] = id.DeepClone();");
            sb.AppendLine("        return payload;");
            sb.AppendLine("    }");
            sb.AppendLine();
        ...
        private string GetClassName() => $"{_entity.EntityName}CrudApiSmokeTests";
        ...
        private string BuildValue(Column column, bool update)
        {
            if (column.IsFK && !string.IsNullOrWhiteSpace(column.FkEntityName))
                return $"ApiSmokeTestContext.GetRequiredCreatedId(\"{Escape(column.FkEntityName)}\", \"{Escape(column.Name)}\")";
