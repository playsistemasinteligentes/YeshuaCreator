...
namespace Input.Repository.Boletim
{
    public partial class BoletimWriteRepository : IBoletimWriteRepository
    {
        private readonly IUnitOfWork _UnitOfWork;
       private readonly IBoletimQueryWrite _query; 

        public BoletimWriteRepository(IUnitOfWork unitOfWork,IBoletimQueryWrite query)
        {
             _UnitOfWork= unitOfWork;
             _query = query;
        }

        public void Insert(IBoletimEntity Boletim)
        {
            var query = _query.InserirBoletimQuery(Boletim);
        Boletim.Id =  _UnitOfWork.ExecuteScalar<int>(query.Query, query.Parameters);
        }

        public void Update(IBoletimEntity Boletim)
        {
            var query = _query.UpdateBoletimQuery(Boletim);
             _UnitOfWork.Execute(query.Query, query.Parameters);
        }
        public void Delete(IBoletimEntity Boletim)
        {
            var query = _query.DeleteBoletimQuery(Boletim);
             _UnitOfWork.Execute(query.Query, query.Parameters);
        }
        ...
        public void UpdateBOL_OBS(int id, string value)
        {
            var query = _query.UpdateBOL_OBS(id, value);
             _UnitOfWork.Execute(query.Query, query.Parameters);
        }
