...
namespace Read.Repository
{
    public partial class LoteTesteReadRepository : ILoteTesteReadRepository
    {
       ...
       protected readonly ILoteTesteQueryRead _query;
        ...
        partial void TryGetLoteTesteCustom(Command.Read.LoteTesteReadCommand command, ref DataPagination<LoteTesteDTO> result, ref bool handled);
        ...
        {
            var customResult = new DataPagination<LoteTesteDTO>();
            var customHandled = false;
            TryGetLoteTesteCustom(command, ref customResult, ref customHandled);
            if (customHandled)
                return customResult;
        ...
        private IEnumerable<LoteTesteTenantIDDTO> getLoteTesteReadFKTenantID(Command.Patterns.Command.SearchFKCommand command )
        {
            var query = _query.LoteTesteTenantIDQuery(command );
                ...
                var lista = _unitOfWork.Query<LoteTesteTenantIDDTO>(query.Query,query.Parameters).ToList();
            return lista;
        }
        ...
        private IEnumerable<LoteTesteUserIdDTO> getLoteTesteReadFKUserId(Command.Patterns.Command.SearchFKCommand command )
        {
            var query = _query.LoteTesteUserIdQuery(command );
                ...
                var lista = _unitOfWork.Query<LoteTesteUserIdDTO>(query.Query,query.Parameters).ToList();
            return lista;
        }
        ...
        public IEnumerable<LoteTesteDTO> GetAllById(int value )
        {
            var query = _query.FirstByIdQuery(value );
                ...
                var result = _unitOfWork.Query<LoteTesteDTO>(query.Query,query.Parameters).ToList();
                return result;
        }
