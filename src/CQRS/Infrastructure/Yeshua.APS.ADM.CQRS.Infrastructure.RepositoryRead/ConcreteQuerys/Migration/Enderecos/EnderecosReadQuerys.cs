...
{
    public class EnderecosQueryRead : QueryBase, IEnderecosQueryRead
    {
        protected readonly IExecutionContext _executionContext;
        public EnderecosQueryRead(IExecutionContext executionContext)
        {
        ...
        public QueryModel EnderecosQuery(Command.Read.EnderecosReadCommand Command )
        {
            ...
            this.Query = $@" select [END_ID], [END_GRUPO], [OperationalEntityId], [TenantID], [Deleted], [Changed], [UserId] from [Enderecos] ";
if (!string.IsNullOrEmpty(Command.END_ID)) dict["END_ID"] = $"%{Command.END_ID}%";
if (!string.IsNullOrEmpty(Command.END_ID)) whereClauses.Add($"[END_ID] like @END_ID");
if (!string.IsNullOrEmpty(Command.END_GRUPO)) dict["END_GRUPO"] = $"%{Command.END_GRUPO}%";
if (!string.IsNullOrEmpty(Command.END_GRUPO)) whereClauses.Add($"[END_GRUPO] like @END_GRUPO");
...
if (Command.UserId.HasValue) whereClauses.Add($"[UserId] = @UserId");
            ...
            Query += " ORDER BY [END_ID] OFFSET @Offset ROWS FETCH NEXT @PageSize ROWS ONLY"; 
 ...
 whereClauses.Add($"[Deleted] = @Deleted");
        ...
        public QueryModel ExistsByEND_IDQuery(string value )
        {
            ...
            dynamic parameters = new ExpandoObject();
            var dict = (IDictionary<string, object>)parameters;
            this.Query = $"SELECT 1 FROM [Enderecos] ";
 dict["TenantID"] = _executionContext.TenantID;
 whereClauses.Add($"[TenantID] = @TenantID");
 dict["Deleted"] = 0;
 whereClauses.Add($"[Deleted] = @Deleted");
                      dict["END_ID"] = value; //04
                      whereClauses.Add($" [END_ID] = @END_ID ");//04
            if (whereClauses.Any()) 
            this.Query += $" WHERE ({string.Join(" AND ", whereClauses)})"; 
            this.Parameters = parameters;
 ...
 dict["Deleted"] = 0;
 whereClauses.Add($"[Deleted] = @Deleted");
                      dict["END_GRUPO"] = value; //04
                      whereClauses.Add($" [END_GRUPO] = @END_GRUPO ");//04
            if (whereClauses.Any()) 
            this.Query += $" WHERE ({string.Join(" AND ", whereClauses)})"; 
            this.Parameters = parameters;
            return new QueryModel(this.Query, parameters);
 ...
 dict["Deleted"] = 0;
 whereClauses.Add($"[Deleted] = @Deleted");
                      dict["END_ID"] = value; //06
                      whereClauses.Add($" [END_ID] = @END_ID ");//06
            if (whereClauses.Any()) 
            this.Query += $" WHERE ({string.Join(" AND ", whereClauses)})"; 
            this.Parameters = parameters;
            return new QueryModel(this.Query, parameters);
 ...
 dict["Deleted"] = 0;
 whereClauses.Add($"[Deleted] = @Deleted");
                      dict["END_GRUPO"] = value; //06
                      whereClauses.Add($" [END_GRUPO] = @END_GRUPO ");//06
            if (whereClauses.Any()) 
            this.Query += $" WHERE ({string.Join(" AND ", whereClauses)})"; 
            this.Parameters = parameters;
            return new QueryModel(this.Query, parameters);
