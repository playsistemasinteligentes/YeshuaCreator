...
function buildUpdate(state) {
    ...
    document.querySelectorAll('.operational-component').forEach(input => targets.push({ component: input.dataset.component, level: input.checked ? 'Information' : 'None', depth: input.dataset.component === 'DomainTracker' ? 'D1' : 'D0' }));
    document.querySelectorAll('.operational-entity-toggle:checked').forEach(input => targets.push({ component: 'DomainTracker', entity: input.dataset.entity, level: 'Information', depth: 'D1' }));
    document.querySelectorAll('.operational-field:checked').forEach(input => targets.push({ component: 'DomainTracker', entity: input.dataset.entity, field: input.dataset.field, level: 'Information', depth: 'D1' }));
    return { defaultLevel: document.getElementById('operational-default-level').value, defaultDepth: document.getElementById('operational-default-depth').value, targets };
}
