...
namespace Command.Receivers
{
    public partial class AutorizarMDFeNaSefazHandler
    {
        ...
        public AutorizarMDFeNaSefazHandler(IyInboxWriteRepository inboxWriteRepository, ILogger logger)
        {
        ...
        partial void CustomExecute(SagaBase saga, SagaStepBase step)
        {
            _inboxWriteRepository.Insert(FiscalSagaPayloads.CreateInbox(
                ...
                saga,
                step,
                "fiscal.mdfe.autorizado-simulado",
                new { origem = "Fiscal", modo = "simulacao-sefaz", cStat = 100, entityId = saga.EntityId }));
        }

        partial void CustomApplyResponse(SagaBase saga, SagaStepBase step, string payload)
        {
            _logger.Info($"Fiscal {saga.EntityId}: MDF-e autorizado em simulacao.");
        }
    }
}
