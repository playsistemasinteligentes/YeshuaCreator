        ...
        partial void CustomActionHook(ref State<CreateContaOutputCommand> state, CreateContaInputCommand comand)
{
            try
            {
                ...
                if (string.IsNullOrEmpty(comand.CpfCnpj))
                    throw new ReceiverException<CreateContaOutputCommand>(Error("Cpf / Cnpj � obrigat�rio", default));

                if (string.IsNullOrWhiteSpace(comand.email))
                    throw new ReceiverException<CreateContaOutputCommand>(Error("Email � obrigat�rio", default));

                if (string.IsNullOrWhiteSpace(comand.nome))
                    throw new ReceiverException<CreateContaOutputCommand>(Error("Nome � obrigat�rio", default));

                if (comand.password != comand.confirmpassword)
                    throw new ReceiverException<CreateContaOutputCommand>(Error("Senhas n�o conferem", default));

                if (_repReadY_Tenant.ExistsByCnpjCpf(comand.CpfCnpj))
                    throw new ReceiverException<CreateContaOutputCommand>(Error("Conta j� existente.", default));
                ...
                if (!tenant.Id.HasValue)
                    throw new ReceiverException<CreateContaOutputCommand>(Error("Erro ao criar Tenant, Id n�o gerado", default));

                // Cria��o do User usando Factory (padr�o seu)
                var user = new yUserFactory(_logger).Create(
                    null,
                    comand.email,
                    comand.email, // Nome: Aqui voc� decide o valor real, coloquei email como exemplo
                    comand.password);
