...
namespace Command.Receivers.Read
{
    public class yUserReadFKTenantIDReceiver : ReciverBase<ICommand, IEnumerable<yUserTenantIDDTO>>
    {
        ...
        protected override Task<State<IEnumerable<yUserTenantIDDTO>>> ActionAsync(ICommand comand, CancellationToken cancellationToken = default)
        {
            if(comand is SearchFKCommand c) 
             {    
                var yUserReadRepository = _repository.getyUserReadFKTenantID(c);
                return Task.FromResult(Success("OK", yUserReadRepository));
            }
            else 
            {
