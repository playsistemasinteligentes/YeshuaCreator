...
{
    public class UpdateTargetProdutoReceiver : ReciverBase<ICommand, ITargetProdutoEntity>
    {
        ...
        public UpdateTargetProdutoReceiver(
        ...
        protected override Task<State<ITargetProdutoEntity>> ActionAsync(ICommand comand, CancellationToken cancellationToken = default)
        {
             if(comand is Command.Write.TargetProdutoCrudCommand c) 
             {    
                 var context = DomainOperationContext.Create(DomainOperation.Alteracao, DomainEntryPoint.Crud, "UpdateTargetProduto", _executionContext.TenantID, _executionContext.UserId, traceId: _executionContext.TraceId, receiverName: nameof(UpdateTargetProdutoReceiver), commandName: "Command.Write.TargetProdutoCrudCommand");
                 ...
                 System.Diagnostics.Activity.Current?.SetTag("yeshua.operational_entity_id", targetproduto.OperationalEntityId);
                 ...
                 try
                 {
                     _repository.Update(targetproduto);
                     return Task.FromResult(Success("OK", targetproduto));
                 }
                 catch (Exception e)
                 {
                    return Task.FromResult(Error(e, targetproduto));
                 }
            }
            else 
            {
