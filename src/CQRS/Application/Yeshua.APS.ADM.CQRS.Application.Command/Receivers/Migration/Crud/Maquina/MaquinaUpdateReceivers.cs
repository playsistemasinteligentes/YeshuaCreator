...
{
    public class UpdateMaquinaReceiver : ReciverBase<ICommand, IMaquinaEntity>
    {
        ...
        protected override Task<State<IMaquinaEntity>> ActionAsync(ICommand comand, CancellationToken cancellationToken = default)
        {
             if(comand is Command.Write.MaquinaCrudCommand c) 
             {    
                 var context = DomainOperationContext.Create(DomainOperation.Alteracao, DomainEntryPoint.Crud, "UpdateMaquina", _executionContext.TenantID, _executionContext.UserId, traceId: _executionContext.TraceId, receiverName: nameof(UpdateMaquinaReceiver), commandName: "Command.Write.MaquinaCrudCommand");
                 ...
                 System.Diagnostics.Activity.Current?.SetTag("yeshua.operational_entity_id", maquina.OperationalEntityId);
                 ...
                 try
                 {
                     _repository.Update(maquina);
                     return Task.FromResult(Success("OK", maquina));
                 }
                 catch (Exception e)
                 {
                    return Task.FromResult(Error(e, maquina));
                 }
            }
            else 
            {
