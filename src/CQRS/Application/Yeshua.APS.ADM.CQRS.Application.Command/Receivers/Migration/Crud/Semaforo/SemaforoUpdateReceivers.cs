...
{
    public class UpdateSemaforoReceiver : ReciverBase<ICommand, ISemaforoEntity>
    {
        ...
        protected override async Task<State<ISemaforoEntity>> ActionAsync(ICommand comand, CancellationToken cancellationToken = default)
        {
             ...
             {    
                 var context = DomainOperationContext.Create(DomainOperation.Alteracao, DomainEntryPoint.Crud, "UpdateSemaforo", _executionContext.TenantID, _executionContext.UserId, traceId: _executionContext.TraceId, receiverName: nameof(UpdateSemaforoReceiver), commandName: "Command.Write.SemaforoCrudCommand");
