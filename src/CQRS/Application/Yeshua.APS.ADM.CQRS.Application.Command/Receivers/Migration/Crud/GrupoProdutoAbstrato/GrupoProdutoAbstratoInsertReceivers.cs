...
{
    public class InsertGrupoProdutoAbstratoReceiver : ReciverBase<ICommand, IGrupoProdutoAbstratoEntity>
    {
        ...
        public InsertGrupoProdutoAbstratoReceiver(
        ...
        protected override Task<State<IGrupoProdutoAbstratoEntity>> ActionAsync(ICommand comand, CancellationToken cancellationToken = default)
        {
             if(comand is Command.Write.GrupoProdutoAbstratoCrudCommand c) 
             {    
                 var context = DomainOperationContext.Create(DomainOperation.Registro, DomainEntryPoint.Crud, "InsertGrupoProdutoAbstrato", _executionContext.TenantID, _executionContext.UserId, traceId: _executionContext.TraceId, receiverName: nameof(InsertGrupoProdutoAbstratoReceiver), commandName: "Command.Write.GrupoProdutoAbstratoCrudCommand");
                 ...
                 System.Diagnostics.Activity.Current?.SetTag("yeshua.operational_entity_id", grupoprodutoabstrato.OperationalEntityId);
                 ...
                 try
                 {
                     _repository.Insert(grupoprodutoabstrato);
                     return Task.FromResult(Success("OK", grupoprodutoabstrato));
                 }
                 catch (Exception e)
                 {
                    return Task.FromResult(Error(e, grupoprodutoabstrato));
                 }
            }
            else 
            {
