...
{
    public class UpdateProdutoReceiver : ReciverBase<ICommand, IProdutoEntity>
    {
        ...
        protected override Task<State<IProdutoEntity>> ActionAsync(ICommand comand, CancellationToken cancellationToken = default)
        {
             if(comand is Command.Write.ProdutoCrudCommand c) 
             {    
                 var context = DomainOperationContext.Create(DomainOperation.Alteracao, DomainEntryPoint.Crud, "UpdateProduto", _executionContext.TenantID, _executionContext.UserId, traceId: _executionContext.TraceId, receiverName: nameof(UpdateProdutoReceiver), commandName: "Command.Write.ProdutoCrudCommand");
                 ...
                 System.Diagnostics.Activity.Current?.SetTag("yeshua.operational_entity_id", produto.OperationalEntityId);
                 ...
                 try
                 {
                     ...
                     return Task.FromResult(Success("OK", produto));
                 }
                 catch (Exception e)
                 {
                    return Task.FromResult(Error(e, produto));
                 }
            }
            else 
            {
