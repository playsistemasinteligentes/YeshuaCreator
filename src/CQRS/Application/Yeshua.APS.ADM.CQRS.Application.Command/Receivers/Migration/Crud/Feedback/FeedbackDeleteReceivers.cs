...
{
    public class DeleteFeedbackReceiver : ReciverBase<ICommand, IFeedbackEntity>
    {
        ...
        protected override Task<State<IFeedbackEntity>> ActionAsync(ICommand comand, CancellationToken cancellationToken = default)
        {
             if(comand is Command.Write.FeedbackCrudCommand c) 
             {    
                 var context = DomainOperationContext.Create(DomainOperation.Remocao, DomainEntryPoint.Crud, "DeleteFeedback", _executionContext.TenantID, _executionContext.UserId, traceId: _executionContext.TraceId, receiverName: nameof(DeleteFeedbackReceiver), commandName: "Command.Write.FeedbackCrudCommand");
                 ...
                 try
                 {
                     ...
                     return Task.FromResult(Success("OK", feedback));
                 }
                 catch (Exception e)
                 {
                    return Task.FromResult(Error(e, feedback));
                 }
            }
            else 
            {
