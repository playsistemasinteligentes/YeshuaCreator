...
{
    public class UpdateOrderReceiver : ReciverBase<ICommand, IOrderEntity>
    {
        ...
        protected override Task<State<IOrderEntity>> ActionAsync(ICommand comand, CancellationToken cancellationToken = default)
        {
             if(comand is Command.Write.OrderCrudCommand c) 
             {    
                 var context = DomainOperationContext.Create(DomainOperation.Alteracao, DomainEntryPoint.Crud, "UpdateOrder", _executionContext.TenantID, _executionContext.UserId, traceId: _executionContext.TraceId, receiverName: nameof(UpdateOrderReceiver), commandName: "Command.Write.OrderCrudCommand");
                 ...
                 System.Diagnostics.Activity.Current?.SetTag("yeshua.operational_entity_id", order.OperationalEntityId);
                 ...
                 try
                 {
                     ...
                     return Task.FromResult(Success("OK", order));
                 }
                 catch (Exception e)
                 {
                    return Task.FromResult(Error(e, order));
                 }
            }
            else 
            {
