...
{
    public class UpdateRoteiroPedidoReceiver : ReciverBase<ICommand, IRoteiroPedidoEntity>
    {
        ...
        public UpdateRoteiroPedidoReceiver(
        ...
        protected override Task<State<IRoteiroPedidoEntity>> ActionAsync(ICommand comand, CancellationToken cancellationToken = default)
        {
             if(comand is Command.Write.RoteiroPedidoCrudCommand c) 
             {    
                 var context = DomainOperationContext.Create(DomainOperation.Alteracao, DomainEntryPoint.Crud, "UpdateRoteiroPedido", _executionContext.TenantID, _executionContext.UserId, traceId: _executionContext.TraceId, receiverName: nameof(UpdateRoteiroPedidoReceiver), commandName: "Command.Write.RoteiroPedidoCrudCommand");
                 ...
                 try
                 {
                     _repository.Update(roteiropedido);
                     return Task.FromResult(Success("OK", roteiropedido));
                 }
                 catch (Exception e)
                 {
                    return Task.FromResult(Error(e, roteiropedido));
                 }
            }
            else 
            {
