...
{
    public class UpdateCanhotosReceiver : ReciverBase<ICommand, ICanhotosEntity>
    {
        ...
        protected override async Task<State<ICanhotosEntity>> ActionAsync(ICommand comand, CancellationToken cancellationToken = default)
        {
             ...
             {    
                 var context = DomainOperationContext.Create(DomainOperation.Alteracao, DomainEntryPoint.Crud, "UpdateCanhotos", _executionContext.TenantID, _executionContext.UserId, traceId: _executionContext.TraceId, receiverName: nameof(UpdateCanhotosReceiver), commandName: "Command.Write.CanhotosCrudCommand");
