...
{
    public class InsertParamReceiver : ReciverBase<ICommand, IParamEntity>
    {
        ...
        protected override async Task<State<IParamEntity>> ActionAsync(ICommand comand, CancellationToken cancellationToken = default)
        {
             ...
             {    
                 var context = DomainOperationContext.Create(DomainOperation.Registro, DomainEntryPoint.Crud, "InsertParam", _executionContext.TenantID, _executionContext.UserId, traceId: _executionContext.TraceId, receiverName: nameof(InsertParamReceiver), commandName: "Command.Write.ParamCrudCommand");
