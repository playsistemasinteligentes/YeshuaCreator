...
{
    public class UpdateUsuarioReceiver : ReciverBase<ICommand, IUsuarioEntity>
    {
        ...
        protected override async Task<State<IUsuarioEntity>> ActionAsync(ICommand comand, CancellationToken cancellationToken = default)
        {
             ...
             {    
                 var context = DomainOperationContext.Create(DomainOperation.Alteracao, DomainEntryPoint.Crud, "UpdateUsuario", _executionContext.TenantID, _executionContext.UserId, traceId: _executionContext.TraceId, receiverName: nameof(UpdateUsuarioReceiver), commandName: "Command.Write.UsuarioCrudCommand");
