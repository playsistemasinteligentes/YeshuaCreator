...
{
    public class UpdateCargaReceiver : ReciverBase<ICommand, ICargaEntity>
    {
        ...
        protected override Task<State<ICargaEntity>> ActionAsync(ICommand comand, CancellationToken cancellationToken = default)
        {
             if(comand is Command.Write.CargaCrudCommand c) 
             {    
                 var context = DomainOperationContext.Create(DomainOperation.Alteracao, DomainEntryPoint.Crud, "UpdateCarga", _executionContext.TenantID, _executionContext.UserId, traceId: _executionContext.TraceId, receiverName: nameof(UpdateCargaReceiver), commandName: "Command.Write.CargaCrudCommand");
                 ...
                 System.Diagnostics.Activity.Current?.SetTag("yeshua.operational_entity_id", carga.OperationalEntityId);
                 ...
                 try
                 {
                     ...
                     return Task.FromResult(Success("OK", carga));
                 }
                 catch (Exception e)
                 {
                    return Task.FromResult(Error(e, carga));
                 }
            }
            else 
            {
