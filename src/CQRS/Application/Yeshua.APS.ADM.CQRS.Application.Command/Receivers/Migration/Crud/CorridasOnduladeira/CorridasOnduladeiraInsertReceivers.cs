...
{
    public class InsertCorridasOnduladeiraReceiver : ReciverBase<ICommand, ICorridasOnduladeiraEntity>
    {
        ...
        public InsertCorridasOnduladeiraReceiver(
        ...
        protected override Task<State<ICorridasOnduladeiraEntity>> ActionAsync(ICommand comand, CancellationToken cancellationToken = default)
        {
             if(comand is Command.Write.CorridasOnduladeiraCrudCommand c) 
             {    
                 var context = DomainOperationContext.Create(DomainOperation.Registro, DomainEntryPoint.Crud, "InsertCorridasOnduladeira", _executionContext.TenantID, _executionContext.UserId, traceId: _executionContext.TraceId, receiverName: nameof(InsertCorridasOnduladeiraReceiver), commandName: "Command.Write.CorridasOnduladeiraCrudCommand");
                 ...
                 System.Diagnostics.Activity.Current?.SetTag("yeshua.operational_entity_id", corridasonduladeira.OperationalEntityId);
                 ...
                 try
                 {
                     _repository.Insert(corridasonduladeira);
                     return Task.FromResult(Success("OK", corridasonduladeira));
                 }
                 catch (Exception e)
                 {
                    return Task.FromResult(Error(e, corridasonduladeira));
                 }
            }
            else 
            {
