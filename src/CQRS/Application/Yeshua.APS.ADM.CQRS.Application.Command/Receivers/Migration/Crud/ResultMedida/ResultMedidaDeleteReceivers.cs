...
{
    public class DeleteResultMedidaReceiver : ReciverBase<ICommand, IResultMedidaEntity>
    {
        ...
        public DeleteResultMedidaReceiver(
        ...
        protected override Task<State<IResultMedidaEntity>> ActionAsync(ICommand comand, CancellationToken cancellationToken = default)
        {
             ...
             {    
                 var context = DomainOperationContext.Create(DomainOperation.Remocao, DomainEntryPoint.Crud, "DeleteResultMedida", _executionContext.TenantID, _executionContext.UserId, traceId: _executionContext.TraceId, receiverName: nameof(DeleteResultMedidaReceiver), commandName: "Command.Write.ResultMedidaCrudCommand");
                 var resultmedida = new ResultMedidaFactory(_logger, _domainTrackingPolicy).Create(context, c.Id, c.RSM_ID, c.RL_ID, c.MDT_ID);
                 System.Diagnostics.Activity.Current?.SetTag("yeshua.operational_entity_id", resultmedida.OperationalEntityId);
                 ...
                 try
                 {
                     _repository.Delete(resultmedida);
