...
namespace Command.Receivers.Read
{
    public class CalendarioReadFKUserIdReceiver : ReciverBase<ICommand, IEnumerable<CalendarioUserIdDTO>>
    {
        ...
        protected override Task<State<IEnumerable<CalendarioUserIdDTO>>> ActionAsync(ICommand comand, CancellationToken cancellationToken = default)
        {
            if(comand is SearchFKCommand c) 
             {    
                var CalendarioReadRepository = _repository.getCalendarioReadFKUserId(c);
                return Task.FromResult(Success("OK", CalendarioReadRepository));
            }
            else 
            {
