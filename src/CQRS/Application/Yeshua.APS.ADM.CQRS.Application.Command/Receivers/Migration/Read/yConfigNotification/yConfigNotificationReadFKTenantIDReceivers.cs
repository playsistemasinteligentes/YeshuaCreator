...
namespace Command.Receivers.Read
{
    public class yConfigNotificationReadFKTenantIDReceiver : ReciverBase<ICommand, IEnumerable<yConfigNotificationTenantIDDTO>>
    {
        ...
        protected override Task<State<IEnumerable<yConfigNotificationTenantIDDTO>>> ActionAsync(ICommand comand, CancellationToken cancellationToken = default)
        {
            if(comand is SearchFKCommand c) 
             {    
                var yConfigNotificationReadRepository = _repository.getyConfigNotificationReadFKTenantID(c);
                return Task.FromResult(Success("OK", yConfigNotificationReadRepository));
            }
            else 
            {
