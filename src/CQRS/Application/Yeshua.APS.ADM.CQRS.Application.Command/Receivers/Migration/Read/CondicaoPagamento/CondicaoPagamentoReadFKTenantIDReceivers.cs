...
namespace Command.Receivers.Read
{
    public class CondicaoPagamentoReadFKTenantIDReceiver : ReciverBase<ICommand, IEnumerable<CondicaoPagamentoTenantIDDTO>>
    {
        ...
        protected override Task<State<IEnumerable<CondicaoPagamentoTenantIDDTO>>> ActionAsync(ICommand comand, CancellationToken cancellationToken = default)
        {
            if(comand is SearchFKCommand c) 
             {    
                var CondicaoPagamentoReadRepository = _repository.getCondicaoPagamentoReadFKTenantID(c);
                return Task.FromResult(Success("OK", CondicaoPagamentoReadRepository));
            }
            else 
            {
