...
namespace Command.Receivers.Read
{
    public class ConsultaPedidoReadFKProdutoIdReceiver : ReciverBase<ICommand, IEnumerable<ConsultaPedidoProdutoIdDTO>>
    {
        ...
        protected override Task<State<IEnumerable<ConsultaPedidoProdutoIdDTO>>> ActionAsync(ICommand comand, CancellationToken cancellationToken = default)
        {
            if(comand is SearchFKCommand c) 
             {    
                var ConsultaPedidoReadRepository = _repository.getConsultaPedidoReadFKProdutoId(c);
                return Task.FromResult(Success("OK", ConsultaPedidoReadRepository));
            }
            else 
            {
