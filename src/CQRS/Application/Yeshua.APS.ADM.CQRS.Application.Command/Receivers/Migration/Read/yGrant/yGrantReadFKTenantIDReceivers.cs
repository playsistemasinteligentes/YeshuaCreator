...
namespace Command.Receivers.Read
{
    public class yGrantReadFKTenantIDReceiver : ReciverBase<ICommand, IEnumerable<yGrantTenantIDDTO>>
    {
        ...
        protected override Task<State<IEnumerable<yGrantTenantIDDTO>>> ActionAsync(ICommand comand, CancellationToken cancellationToken = default)
        {
            if(comand is SearchFKCommand c) 
             {    
                var yGrantReadRepository = _repository.getyGrantReadFKTenantID(c);
                return Task.FromResult(Success("OK", yGrantReadRepository));
            }
            else 
            {
