...
namespace Command.Receivers.Read
{
    public class ClinicaReadFKTenantIDReceiver : ReciverBase<ICommand, IEnumerable<ClinicaTenantIDDTO>>
    {
        ...
        protected override Task<State<IEnumerable<ClinicaTenantIDDTO>>> ActionAsync(ICommand comand, CancellationToken cancellationToken = default)
        {
            if(comand is SearchFKCommand c) 
             {    
                var ClinicaReadRepository = _repository.getClinicaReadFKTenantID(c);
                return Task.FromResult(Success("OK", ClinicaReadRepository));
            }
            else 
            {
