...
namespace Command.Receivers.Read
{
    public class PlanoContaReadFKTenantIDReceiver : ReciverBase<ICommand, IEnumerable<PlanoContaTenantIDDTO>>
    {
        ...
        protected override Task<State<IEnumerable<PlanoContaTenantIDDTO>>> ActionAsync(ICommand comand, CancellationToken cancellationToken = default)
        {
            if(comand is SearchFKCommand c) 
             {    
                var PlanoContaReadRepository = _repository.getPlanoContaReadFKTenantID(c);
                return Task.FromResult(Success("OK", PlanoContaReadRepository));
            }
            else 
            {
